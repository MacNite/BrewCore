...
// Must match PASSWORD_CHANGE_COOKIE in src/lib/auth.ts, which cannot be
// imported here: it pulls in argon2, and middleware runs on the edge.
const PASSWORD_CHANGE_COOKIE = "brewcore_password_change";
...
function gate(request: NextRequest, requestHeaders: Headers) {
  ...
  if (request.cookies.get(PASSWORD_CHANGE_COOKIE)?.value !== "1") return next();

  const { pathname } = request.nextUrl;
  const allowed = pathname === "/change-password" || pathname === "/login" || pathname === "/offline" || pathname.startsWith("/_next/");
 ...
 */
export const config = {
  matcher: ["/((?!api/health|_next/static|_next/image|icon.svg|manifest.webmanifest|sw.js|.*\\.(?:png|jpg|svg|ico)$).*)"],
};
