...
describe("request body limit", () => {
  it("covers two images plus multipart overhead", () => {
    // One bag photo is the largest request the application makes.
    expect(requestBodyLimitMb("5")).toBe(6);
    expect(requestBodyLimitMb(undefined)).toBe(6);
  });
  ...
  it("stays bounded by the configured ceiling", () => {
    ...
    expect(requestBodyLimitMb("999")).toBe(MAX_CONFIGURED_IMAGE_UPLOAD_MB + 1);
  });
});
